 ...
 public:
  vector<vector<int>> verticalTraversal(TreeNode* root) {
    ...
    while (!q.empty()) {
      ...
      m[current_offset][current_level].push_back(
          current_node->val);  // Insert into its position
    ...
    for (pair<int, map<int, vector<int>>> outer_map : m) {
      ...
      vector<int> vertical_traversal_offset_temp;  // All offsets need to be in
                                                   // a single vector

      for (pair<int, vector<int>> p2 : inner_map) {
        vector<int> level_in_offset = p2.second;
        sort(level_in_offset.begin(),
             level_in_offset.end());  // Sorting only occurs within a level
